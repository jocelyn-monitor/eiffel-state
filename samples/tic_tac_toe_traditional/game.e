	...
	has_empty: BOOLEAN is
		...
		do
			...
			loop
				...
				loop
					if field.item (i, j) = Empty_code then
						Result := True
					end
					j := j + 1
				end
...
feature {NONE} -- Implementation
	check_horizontal is
		...
		do
			...
			loop
				winner := field.item (i, 1)
				from
					j := 2
				until
					j > Dimension or not has_winner
				loop
					if winner /= field.item (i, j) then
						winner := Empty_code
					end
					j := j + 1
				end
	...
	check_vertical is
		...
		do
			...
			until
				i > Dimension or has_winner
			loop
				winner := field.item (1, i)
				from
					j := 2
				until
					j > Dimension or not has_winner
				loop
					if winner /= field.item (j, i) then
						winner := Empty_code
					end
					j := j + 1
				end
	...
	check_diagonal is
		...
		local
			i: INTEGER
		do
			winner := field.item (1, 1)
			from
				i := 2
			until
				i > 3 or not has_winner
			loop
				if winner /= field.item (i, i) then
					winner := Empty_code
				end
				i := i + 1
			end
			...
			if not has_winner then
				winner := field.item (1, Dimension)
				from
					i := 2
				until
					i > 3
				loop
					if winner /= field.item (i, Dimension + 1 - i) then
						winner := Empty_code
					end
					i := i + 1
				end
