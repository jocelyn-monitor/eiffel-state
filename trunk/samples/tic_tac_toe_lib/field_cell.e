...
feature {NONE} -- Automaton
	...
	sd_out: STATE_DEPENDENT_FUNCTION [TUPLE, STRING] is
		...
		once
			...
			Result.add_result (Empty, agent : BOOLEAN do Result := True end, "")
			Result.add_result (Cross, agent : BOOLEAN do Result := True end, "X")
			Result.add_result (Circle, agent : BOOLEAN do Result := True end, "O")
		end
